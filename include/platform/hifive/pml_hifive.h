...
#define PLIC_SET_MENABLE(hart,intID)  do {                                              \
        PLIC_MENABLE_ARRAYBASE((hart))[ (intID) / 32] |= (1 << ((intID) % 32));         \
    } while (0);                                                                        
...
#define PLIC_SET_SENABLE(hart,intID)  do {                                              \
        PLIC_SENABLE_ARRAYBASE((hart))[ (intID) / 32] |= (1 << ((intID) % 32));         \
    } while (0);                                                                                                                              \
    // ASSERT((hart) > 0);                                             
